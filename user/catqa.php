...
$obj1=new conclass();
$res1=$obj1->getdata("select * from que_tbl where flag='$flag' and fk_cat_id='$cid' LIMIT {$page1}, {$noi}");
...
//			$desc=$row["q_desc"];
	...
	//x		echo '<p class="question-desc" >'.$desc.'</p>';
		...
		$res1=$obj->getdata("select * from que_tbl where flag=1 and fk_cat_id='$cid'");
...
<?php
			...
			{	
			echo '<a href="catqa.php?page='.$first_page.'&id='.$cid.'" style="text-decoration:none;"><button class="button"><<</button></a>';	
			}
			if($prev_page==0)
			{
			...
			{
		echo '<a href="catqa.php?page='.$prev_page.'&id='.$cid.'" style="text-decoration:none;"><button class="button">Previous</button></a>';	
			}
			
			for($b=1;$b<=$a;$b++)
		{
			echo '<a href="catqa.php?page='.$b.'&id='.$cid.'" style="text-decoration:none;"><button class="button">'.$b.'</button></a>'; 
		}
		
		if($next_page==$a)
		{
			echo '<a href="catqa.php?page='.$next_page.'&id='.$cid.'" style="text-decoration:none;"><button class="button">Next</button></a>';	
		}
		else
		{	
		...
		{	
		echo '<a href="catqa.php?page='.$last_page.'&id='.$cid.'" style="text-decoration:none;"><button class="button">>></button></a>';
		}
		echo '</center>';
			}
		?>
