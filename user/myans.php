...
$flag=1;	
	$obj=new conclass();
	$res1=$obj1->getdata("select * from que_tbl as q , ans_tbl as a where a.fk_q_id=q.pk_q_id and a.fk_email_id='$email'  LIMIT {$page1}, {$noi}");
...
//			$desc=$row["q_desc"];
	...
	//x		echo '<p class="question-desc" >'.$desc.'</p>';
		...
		$res1=$obj->getdata("select * from que_tbl as q , ans_tbl as a where a.fk_q_id=q.pk_q_id and a.fk_email_id='$email'");
...
<?php
			...
			{	
			echo '<a href="myans.php?page='.$first_page.'" style="text-decoration:none;"><button class="button"><<</button></a>';	
			}
			if($prev_page==0)
			{
			...
			{
		echo '<a href="myans.php?page='.$prev_page.'" style="text-decoration:none;"><button class="button">Previous</button></a>';	
			}
			
			for($b=1;$b<=$a;$b++)
		{
			echo '<a href="myans.php?page='.$b.'" style="text-decoration:none;"><button class="button">'.$b.'</button></a>'; 
		}
		
		if($next_page==$a)
		{
			echo '<a href="myans.php?page='.$next_page.'" style="text-decoration:none;"><button class="button">Next</button></a>';	
		}
		else
		{	
		...
		{	
		echo '<a href="myans.php?page='.$last_page.'" style="text-decoration:none;"><button class="button">>></button></a>';
		}
		echo '</center>';
			}
		?>
